...
switch ($_GET["op"]) {
	...
	case 'desactivar':
		...
		echo $rspta ? "Datos borrados correctamente" : "No se pudo desactivar los datos";
		break;
	case 'activar':
    ...
    case 'listar':
		...
		while ($reg=$rspta->fetch_object()) {
			$data[]=array(
            "0"=>'<button class="btn btn-warning btn-xs" onclick="mostrar('.$reg->id.')"><i class="fa fa-pencil-square-o"></i></button>',
            "1"=>$reg->name
              );
		}
		$results=array(
