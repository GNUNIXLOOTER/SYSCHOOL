...
switch ($_GET["op"]) {
	case 'guardaryeditar':
		...
		if (empty($idusuario)) {
			$rspta=$empleados->insertar($nombre,$tipo_documento,$num_documento,$direccion,$telefono,$email,$cargo,$salario,$imagen);
			echo $rspta ? "Datos registrados correctamente" : "Datos registrados correctamente";
		}
		else {
			$rspta=$empleados->editar($idusuario,$nombre,$tipo_documento,$num_documento,$direccion,$telefono,$email,$cargo,$salario,$imagen,);
	...
	case 'desactivar':
		$rspta=$empleados->desactivar($idusuario);
		echo $rspta ? "Datos desactivados correctamente" : "No se pudo desactivar los datos";
	break;

	case 'activar':
		$rspta=$empleados->activar($idusuario);
		echo $rspta ? "Datos activados correctamente" : "No se pudo activar los datos";
	break;
	
	case 'mostrar':
		$rspta=$empleados->mostrar($idusuario);
	...
	case 'listar':
		...
		while ($reg=$rspta->fetch_object()) {
			$data[]=array(
				...
				"5"=>$reg->email,
				"6"=>$reg->salario,
				"7"=>"<img src='../files/empleados/".$reg->imagen."' height='50px' width='50px'>",
	...
	case 'verificar':
		...
		if (isset($fetch)) 
		{
			...
			$_SESSION['salario']=$fetch->salario;
