...
public function insertar($nombre,$favorito,$idusuario){
	$sql="INSERT INTO grupo (nombre,favorito,idusuario) VALUES ('$nombre','$favorito','$idusuario')";
	return ejecutarConsulta($sql);
}

public function editar($idgrupo,$nombre,$favorito,$idusuario){
	$sql="UPDATE grupo SET nombre='$nombre',favorito='$favorito',idusuario='$idusuario'  
	WHERE idgrupo='$idgrupo'";
	return ejecutarConsulta($sql);
}

public function anular($idgrupo){
	$sql="UPDATE grupo SET estado='Anulado' WHERE idgrupo='$idgrupo'"; 
	return ejecutarConsulta($sql);
}
...
public function mostrar_grupo($idgrupo){
	$sql="SELECT idgrupo,nombre,favorito, idusuario  FROM grupo WHERE idgrupo='$idgrupo'";
	return ejecutarConsulta($sql);
}
...
public function listar(){
	$sql="SELECT g.idgrupo,g.nombre,u.idusuario,u.nombre as usuario FROM grupo g  INNER JOIN usuario u ON g.idusuario=u.idusuario ORDER BY g.idgrupo DESC";
	return ejecutarConsulta($sql);
}


public function ventacabecera($idgrupo){
	$sql= "SELECT g.idgrupo, g.idcliente, p.nombre AS cliente, p.direccion, p.tipo_documento, p.num_documento, p.email, p.telefono, g.idusuario, u.nombre AS usuario, g.tipo_comprobante, g.serie_comprobante, g.num_comprobante, DATE(g.fecha_hora) AS fecha, g.impuesto, g.total_venta FROM grupo g INNER JOIN persona p ON g.idcliente=p.idpersona INNER JOIN usuario u ON g.idusuario=u.idusuario WHERE g.idgrupo='$idgrupo'";
	return ejecutarConsulta($sql);
}
