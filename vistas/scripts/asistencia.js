	...
	$.post("../ajax/asistencia.php?op=verificar",{fecha_asistencia : today, alumn_id:id, idgrupo:idgrupo},
		function(data,status)
		{
				...
				if(data==null && $("#tipo_asistencia").val()!=""){
					 	...
					 	$.post("../ajax/alumnos.php?op=mostrar",{id : id},
						function(data,status)
						{
							data=JSON.parse(data);
...
function mostrar(id){
	$.post("../ajax/alumnos.php?op=mostrar",{id : id},
		function(data,status)
		{
			...
			$("#imagenmuestra").show();
			$("#imagenmuestra").attr("src","../files/imagenes/"+data.image);
...
function desactivar(id){
	bootbox.confirm("¿Esta seguro de desactivar este dato?", function(result){
		if (result) {
			$.post("../ajax/alumnos.php?op=desactivar", {id : id}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}
...
function activar(id){
	bootbox.confirm("¿Esta seguro de activar este dato?" , function(result){
		if (result) {
			$.post("../ajax/alumnos.php?op=activar" , {id : id}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}
